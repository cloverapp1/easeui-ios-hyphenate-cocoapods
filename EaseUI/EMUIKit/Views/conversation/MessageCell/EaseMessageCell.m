...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if ([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]] &&
        ...
        NSDictionary *attr = [self.bubbleView.textLabel linkAtPoint:pt];
        if (attr && [attr[KILabelLinkTypeKey] intValue]==KILinkTypeURL) {
            NSURL *url = [NSURL URLWithString:attr[KILabelLinkKey]];
            if (_delegate && [_delegate respondsToSelector:@selector(didSelectLinkWithURL:)]) {
                [_delegate didSelectLinkWithURL:url];
            }
            return NO;
        }
    }
